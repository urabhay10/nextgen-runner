...
export default function CompareModels() {
  ...
  const updatePlayer = (tId: 1 | 2, idx: number, v: string) => {
    ...
    const n = team.players.map((p, i) => i === idx ? { ...p, name: v } : p);
  ...
  if (stage === 'setup') return (
    <div className="min-h-screen p-8 max-w-6xl mx-auto" style={{ background: 'var(--background)' }}>
      <header className="mb-12 text-center relative">
          <Link href="/v1" className="absolute left-0 top-1/2 -translate-y-1/2 transition flex items-center gap-2 text-sm font-bold px-4 py-2 rounded-full border" style={{ color: 'var(--muted)', background: 'var(--surface)', borderColor: 'var(--border)' }}>
