...
export default function DuelPage() {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (updated.status === 'matched' && updated.match_id) {
            router.push(`/v1/duel/${updated.match_id}`);
          }
        }
      )
      .subscribe();
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      router.push(`/v1/duel/${data.match_id}`);
    } catch {
...
<Link href="/v1" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest mb-8 hover:opacity-70 transition-opacity" style={{ color: 'var(--muted)' }}>
